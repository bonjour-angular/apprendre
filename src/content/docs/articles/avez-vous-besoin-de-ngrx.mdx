...
export const loadTodos = createEffect(
  (
    ...
    actions$.pipe(
      ofType(todosActions.todosPageInitialized), // 👈 j'écoutel'action qui m'intéresse
      switchMap(() =>
        http.get<Todo[]>('api/todos').pipe(
